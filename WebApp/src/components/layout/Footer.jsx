...
export default function Footer() {
  ...
  const range = minYear && maxYear ? `${minYear}–${maxYear}` : '2016–2025'
  return (
    <footer className="bg-brand-gray-light/60 border-t border-border">
      <div className="container-chrome py-4 flex items-center justify-center">
        <p className="text-base text-text-secondary text-center">
          Data source: U.S. Customs and Border Protection (CBP), northbound crossings at Texas–Mexico ports of entry, {range}.
        </p>
      </div>
    </footer>
  )
