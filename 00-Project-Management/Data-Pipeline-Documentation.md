...
| File | Coverage | Notes |
|---|---|---|
| `01-Raw-Data/2008-2024 Master_CBP Border Crossings.xlsx` | 2008–2024, all TX crossings, monthly | Total-flow (NB+SB). Wide format: one row per crossing per month, mode columns. |
...
| `Northbound Crossing` | int | Non-negative count. **Note: 2008–2024 values are total-flow (NB+SB) from the Master workbook; 2025 values are northbound only.** |
...
### NB vs total-flow values
The Master workbook (`2008-2024`) reports **total-flow** (northbound + southbound combined). The `Northbound Crossing` column name is retained for schema compatibility. The 2025 data from ELP PDFs and the LRD-RVG workbook is northbound only. Users comparing 2024 and 2025 annual totals should be aware of this difference. See `Raw-Data-Gaps-and-Anomalies.md` for details.
...
| Crossing | First year in data |
|---|---|
| Anzalduas International Bridge | 2010 |
| Donna-Rio Bravo International Bridge | 2010 |
| El Paso Railroad Bridges | 2010 |
| Marcelino Serna Bridge | 2011 |
| Boquillas | 2014 |
